...
def Huawei_p20_sentiments_file():
    s_sentiments = []
    with open(r'Huawei_p20_clean.txt','r',encoding='utf-8') as f:
        for i in f.readlines():
            s = SnowNLP(i)
            s_sentiments.append(s.sentiments)
    with open('Huawei_p20_sentiments.csv','w',encoding='utf-8') as w:
        w.write(str(s_sentiments).strip('[]'))
        w.close()

#词云图制作
def Huawei_p20_wordcloud_plot():
    #出现最多的字段统计，取前200
    with open(r'Huawei_p20_clean.txt', 'r', encoding='utf-8') as f:
...
#情感系数分布图
def Huawei_p20_emotion_plot():

    b = np.loadtxt('Huawei_p20_sentiments.csv', delimiter=',', dtype=np.float64)
...
def Huawei_p20_data_analysis_main():
    matplotlib.rcParams['font.family']='YouYuan'
    matplotlib.rcParams['font.size']=10
    file = os.path.exists('Huawei_p20_sentiments.csv')
    if not file:
        Huawei_p20_sentiments_file()
        Huawei_p20_wordcloud_plot()
        Huawei_p20_emotion_plot()
        Huawei_p20_comment_plot()
    else:
        Huawei_p20_wordcloud_plot()
        Huawei_p20_emotion_plot()
        Huawei_p20_comment_plot()
