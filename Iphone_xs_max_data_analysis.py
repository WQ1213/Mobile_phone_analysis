...
def Iphone_xs_max_sentiments_file():
    s_sentiments = []
    with open(r'Iphone_xs_max_clean.txt','r',encoding='utf-8') as f:
        for i in f.readlines():
            s = SnowNLP(i)
            s_sentiments.append(s.sentiments)
    with open('Iphone_xs_max_sentiments.csv','w',encoding='utf-8') as w:
        w.write(str(s_sentiments).strip('[]'))
        w.close()

#词云图制作
def Iphone_xs_max_wordcloud_plot():
    #出现最多的字段统计，取前200
    with open(r'Iphone_xs_max_clean.txt', 'r', encoding='utf-8') as f:
...
#情感系数分布图
def Iphone_xs_max_emotion_plot():

    b = np.loadtxt('Iphone_xs_max_sentiments.csv', delimiter=',', dtype=np.float64)
...
#评论分布图
def Iphone_xs_max_comment_plot():
    b = np.loadtxt('Iphone_xs_max_sentiments.csv', delimiter=',', dtype=np.float64)
...
def Iphone_xs_max_data_analysis_main():
    matplotlib.rcParams['font.family']='YouYuan'
    matplotlib.rcParams['font.size']=10
    file = os.path.exists('Iphone_xs_max_sentiments.csv')
    if not file:
        Iphone_xs_max_sentiments_file()
        Iphone_xs_max_wordcloud_plot()
        Iphone_xs_max_emotion_plot()
        Iphone_xs_max_comment_plot()
    else:
        Iphone_xs_max_wordcloud_plot()
        Iphone_xs_max_emotion_plot()
        Iphone_xs_max_comment_plot()
